...
class Abs(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        for i in dat.columns:
            if (dat[i].values < 0).any():
                cols.append(str(i) + '_abs')
                d.append(delayed(abs)(dat[i]))
        d = compute(d)[0]
        if not d:
            return pd.DataFrame()
...
class Adde(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            cols.append(str(i) + '_adde')
            d.append(delayed(lambda x: x + np.e)(dat[i]))
        d = compute(d)[0]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
        return res
...
class Cos(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            cols.append(str(i) + '_cos')
            d.append(delayed(lambda x: np.cos(x))(dat[i].astype(float)))
        d = compute(d)[0]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
        return res
...
class Degree(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            cols.append(str(i) + '_degree')
            d.append(delayed(lambda x: np.degrees(x))(dat[i]))
        d = compute(d)[0]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
        return res
...
class Exp(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            if (dat[i].values < 1).all():
                cols.append(str(i) + '_exp')
                d.append(delayed(lambda x: np.exp(x))(dat[i]))
        d = compute(d)[0]
        if not d:
            return pd.DataFrame()
        d = np.transpose(np.array(d, dtype='float32'))
...
class Ln(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            if (dat[i].values > 1e-9).all():
                cols.append(str(i) + '_ln')
                d.append(delayed(lambda x: np.log(x))(dat[i]))
        d = compute(d)[0]
        if not d:
            return pd.DataFrame()
        d = np.transpose(np.array(d, dtype='float32'))
...
class Negative(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            cols.append(str(i) + '_negative')
            d.append(delayed()(-dat[i]))
        d = compute(d)[0]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
        return res
...
class Radian(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            cols.append(str(i) + '_radian')
            d.append(delayed(lambda x: np.radians(x))(dat[i]))
        d = compute(d)[0]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
        return res
...
class Reciprocal(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            cols.append(str(i) + '_recip')
            d.append(delayed(lambda x: 1/x if x.all()!=0 else x)(dat[i]))
        d = compute(d)[0]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
        return res
...
class Sin(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            cols.append(str(i) + '_sin')
            d.append(delayed(lambda x: np.sin(x))(dat[i]))
        d = compute(d)[0]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
        return res
...
class Square(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            cols.append(str(i) + '_square')
            d.append(delayed(lambda x: x * x)(dat[i]))
        d = compute(d)[0]
        idx = ()
        for i in range(len(cols)):
            if (not valid_col(d[i])):
                idx = idx + (i,)
        cols = np.delete(cols, idx).tolist()
        d = [i for j, i in enumerate(d) if j not in idx]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
        return res
...
class Tanh(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            cols.append(str(i) + '_tanh')
            d.append(delayed(lambda x: np.tanh(x))(dat[i]))
        d = compute(d)[0]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
        return res
...
class Relu(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        d = []
        for i in dat.columns:
            cols.append(str(i) + '_relu')
            d.append(delayed(lambda x: x * (x > 0))(dat[i]))
        d = compute(d)[0]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
        return res
...
class KernelApproxRBF(Transform):
    ...
    def _exec(self, dat: pd.DataFrame):
        ...
        res = pd.DataFrame(rbf_feature.fit_transform(dat), index=dat.index, columns=cols)
        return res
...
class Div(Transform):
    ...
    def _exec(self, dat: pd.DataFrame, prev_dat: pd.DataFrame=None):
        ...
        for i in range(len(cols1)):
            for j in np.arange(i+1, len(cols1)):
                if dat[cols1[j]].any() == 0:
                    continue
                d.append(delayed(dat[cols1[i]] / dat[cols1[j]]))
                cols.append('(' + str(cols1[i]) + '/' + str(cols1[j]) + ')')
            ...
            for i in range(len(cols1)):
                for j in range(len(cols2)):
                    if prev_dat[cols2[j]].any()==0:
                        continue
                    d.append(delayed(dat[cols1[i]] / prev_dat[cols2[j]]))
                    cols.append('(' + str(cols1[i]) + '/' + str(cols2[j]) + ')')
        ...
        for i in range(len(cols)):
            if (not valid_col(d[i])):
                idx = idx + (i,)
        # Unable to allocate 78.1 MiB for an array with shape (186043,) and data type <U110 (prev: 481; cur: 281)
        cols = np.delete(cols, idx).tolist()
        d = [i for j, i in enumerate(d) if j not in idx]
        d = np.transpose(np.array(d, dtype='float32'))
        res = pd.DataFrame(d, columns=cols)
...
unary_operators = {'abs':Abs, 'adde':Adde, 'cos':Cos, 'degree':Degree, 'exp':Exp, 'ln':Ln, 'negative':Negative, 
                   'radian':Radian, 'reciprocal':Reciprocal, 'sin':Sin, 'sigmoid':Sigmoid, 'square':Square, 
